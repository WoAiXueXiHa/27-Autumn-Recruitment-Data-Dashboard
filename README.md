...
- 支持搜索、状态筛选和排序。
- 顶部搜索会留在当前模块内，不会因为输入数字或关键词擅自跳转页面。
...
- 纯数字与 `#题号` 使用精确题号匹配，例如 `1` 和 `#1` 都只定位第 1 题。
- 支持官方顺序、题号、难度和下次复习时间排序。
- 点击题名可直接打开对应的力扣中文站题目。
- 可将已经填写的解题思路按题型导出为一个 Markdown 文档。
...
- 导出 Hot 100 思路 Markdown、投递记录 Excel/CSV，或包含全部记录与设置的 JSON 备份。
- 支持浅色、深色和跟随系统主题。

### 模块化搜索与顶部操作

- 秋招投递、面经复盘和 Hot 100 分别维护自己的搜索关键词，切换模块后互不覆盖。
- 清空搜索框会立即恢复当前模块的完整列表。
- 桌面端使用顶部搜索，较窄窗口使用模块内容区搜索，两处始终同步。
- 总览和数据管理不显示搜索框；顶部主按钮仅在投递和面经模块出现，并显示对应操作。
- 顶部栏随页面正常滚动，左侧导航仍保持固定。

### 导出 Hot 100 思路

进入“数据管理”，点击“Hot 100 思路（数量）”。应用会先等待尚未完成的自动保存，然后下载 `hot100-notes-YYYYMMDD.md`。

- 只包含已经填写“解题思路”的题目，不生成空白占位。
- 文件使用一级总标题、二级题型和三级题目标题，并附带难度、类型及力扣链接。
- 笔记中的标题会在导出副本中自动降级，避免破坏文档目录；看板里的原始笔记不会被修改。
- 列表、加粗、行内代码和各种语言代码块会继续保留。
- Markdown 是阅读和分享副本，不能替代用于完整恢复的 JSON 备份。
...
- 发布代码前不要强制添加被 `.gitignore` 排除的 `state.json`、备份、日志或导出的笔记文件。
...
V12 起顶部搜索只作用于当前模块。需要搜索题目时先进入 Hot 100，需要搜索面经时先进入面经复盘；这样数字、公司名和题目名不会被错误分流。
